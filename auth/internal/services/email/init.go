...
type EmailService interface {
	...
	SendWelcomeMessageForPharmacy(ctx context.Context, email, token string) (err error)
...
func (es *emailService) SendWelcomeMessageForPharmacy(ctx context.Context, email, token string) error {
	ctx, span := otel.Tracer(emailErrorTracer).Start(ctx, "SendWelcomeMessageForPharmacy")
	defer span.End()
	...
	var body bytes.Buffer
	if err := es.template.ExecuteTemplate(&body, "welcome_for_pharmacy.html", data); err != nil {
		return ce.NewError(span, ce.CodeEmailTemplateParsing, ce.MsgInternalServer, err)
	}
